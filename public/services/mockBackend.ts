...
import { evaluate3, evaluate5, compareHands, AnalyzedHand, HandType, getLocalSuggestions } from './suggestions';

// --- Storage Keys ---
const STORE_KEY_CARRIAGES = 'thirteen_water_carriages_v1';
const STORE_KEY_SUBMISSIONS = 'thirteen_water_submissions_v1';

// --- Scoring Logic (Duplicated from Backend Logic for Mock) ---
...
// Simplified Special Hand Check for Mock
...
export const submitPlayerHand = (playerId: string, submission: HandSubmission) => {
    const stored = localStorage.getItem(STORE_KEY_SUBMISSIONS);
    const all = stored ? JSON.parse(stored) : [];
    // Remove existing submission for this table by this player
...
export const settleGame = (playerId: string): { totalScore: number, details: TableResult[], validCarriageCount: number } => {
    ...
    // Removed Bot Filling Logic - Settlement only calculates existing submissions
    ...
    Object.keys(map).forEach(cId => {
        ...
        Object.keys(map[cId]).forEach(tId => {
            ...
            // Only settle tables where 'playerId' is involved
            ...
            // Check for valid game (at least 2 players)
            if (subs.length < 2) {
                ...
                details.push({ 
                    ...
                    voided: true // Voided if not enough players
                });
                return;
            }
            ...
            for(let i=0; i<wSubs.length; i++) {
                for(let j=i+1; j<wSubs.length; j++) {
                    ...
                    if(p1.special || p2.special) {
                        ...
                        if(p2W===3) { w*=2; isShoot=true; } // w is negative
            ...
            matchResults.forEach(m => {
                ...
                scores[m.p1] += finalW * 2; // Water -> Points
