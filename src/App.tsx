...
const App: React.FC = () => {
  ...
  const handleSeatSelect = async (carriageId: number, seat: Seat) => {
      ...
      if (currentPoints < 300) {
          alert(`您的积分 (${currentPoints}) 不足 300，无法入座。\n请联系管理员或好友获取积分。`);
          return;
      }
          ...
          <div className="absolute inset-0 z-[60] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
              <div className="bg-green-900 border border-yellow-500/30 w-full max-w-sm rounded-2xl p-6 shadow-2xl relative flex flex-col max-h-[90vh]">
                  ...
                  <div className="flex border-b border-white/10 mb-6 shrink-0">
                      <button onClick={() => setShowAuthModal('login')} className={`flex-1 pb-3 text-lg font-bold transition-all ${showAuthModal === 'login' ? 'text-yellow-400 border-b-2 border-yellow-400' : 'text-white/40'}`}>账号登录</button>
                      <button onClick={() => setShowAuthModal('register')} className={`flex-1 pb-3 text-lg font-bold transition-all ${showAuthModal === 'register' ? 'text-yellow-400 border-b-2 border-yellow-400' : 'text-white/40'}`}>新用户注册</button>
                  </div>
                  <div className="space-y-4 overflow-y-auto p-1">
                      <input type="tel" className="w-full bg-black/40 border border-white/10 rounded-lg px-4 py-3 text-white" value={authForm.phone} onChange={e => setAuthForm({...authForm, phone: e.target.value})} placeholder="请输入手机号" />
                      {showAuthModal === 'register' && <input type="text" className="w-full bg-black/40 border border-white/10 rounded-lg px-4 py-3 text-white" value={authForm.nickname} onChange={e => setAuthForm({...authForm, nickname: e.target.value})} placeholder="昵称" />}
