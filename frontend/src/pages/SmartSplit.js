...
function findPotentialStraights(cards) {
  ...
  for (let i = 0; i <= uniqueVals.length - 5; i++) {
    ...
    if (isStraight) {
      ...
      straightCards.sort((a, b) => cardValue(a) - cardValue(b));
  ...
  const hasLowStraight = lowStraightValues.every(v => uniqueVals.includes(v));

  if (hasLowStraight) {
    const straightCards = cards.filter(card => lowStraightValues.includes(cardValue(card)));
...
function handType(cards, area) {
  ...
  const suits = cards.map(card => card.split('_')[1]);
