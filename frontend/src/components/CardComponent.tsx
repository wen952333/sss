  ...
  return (
    <div
      ...
      className={`
        ...
        ${selected ? 'ring-4 ring-red-500 shadow-xl z-10' : ''}
        ${small ? 'w-10 h-14 text-xs' : 'w-24 h-36 sm:w-32 sm:h-48 text-3xl sm:text-6xl'}
        ${className}
      `}
    >
      ...
      {selected && (
          <div className="absolute -top-2 -right-2 bg-red-500 text-white rounded-full w-6 h-6 flex items-center justify-center text-sm shadow-sm z-20">✓</div>
      )}
    </div>
  );
};