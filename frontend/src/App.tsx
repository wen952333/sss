...
import { Card, GamePhase, GameState, PlayerHand, Seat, User } from './types';
...
const App: React.FC = () => {
  ...
  const [walletForm, setWalletForm] = useState({ searchPhone: '', targetUser: null as { id: number, nickname: string } | null, amount: '' });
  ...
  const handleSearchUser = async () => {
      ...
      try {
          const res = await fetch('/api/user/search', { method: 'POST', body: JSON.stringify({ query: walletForm.searchPhone }) });
          const data = await res.json();
          if (data.found) { setWalletForm(prev => ({ ...prev, targetUser: data.user })); setWalletMsg(''); }
          else { setWalletForm(prev => ({ ...prev, targetUser: null })); setWalletMsg('未找到该用户'); }
      } catch (e) { setWalletMsg('搜索出错'); }
  };
  ...
  const handleTransfer = async () => {
      ...
      try {
          const res = await fetch('/api/user/transfer', {
              method: 'POST',
              body: JSON.stringify({ fromId: gameState.user.id, toId: walletForm.targetUser.id, amount: walletForm.amount })
          });
          const data = await res.json();
          if (data.error) throw new Error(data.error);
  ...
  const handleSeatSelect = (carriageId: number, seat: Seat) => {
      if (lobbySelection?.carriageId === carriageId && lobbySelection?.seat === seat) setLobbySelection(null);
      else setLobbySelection({ carriageId, seat });
  };

  const handleEnterGame = () => {
      if (!lobbySelection) return;
      if (!gameState.user) { setShowAuthModal('login'); return; }
      const { carriageId, seat } = lobbySelection;
      const count = getCarriagePlayerCount(carriageId);
      if (count < 2) { alert("当前场次暂无其他玩家，请稍后重试或等待玩家加入。"); return; }
      ...
      setGameState(prev => ({
        ...prev, phase: GamePhase.PLAYING, currentCarriageId: carriageId, currentTableIndex: 0,
        tableQueue: queue, mySeat: seat, currentCards: cards, currentArrangement: suggestions[0],
        aiSuggestions: suggestions, currentSuggestionIndex: 0
      }));
      setSelectedCardIds([]);
  };
      ...
      setGameState(prev => {
          ...
          (['top', 'middle', 'bottom'] as const).forEach(key => {
              ...
              for (const card of newArrangement[key]) {
                  if (selectedCardIds.includes(card.id)) cardsToMove.push(card); else staying.push(card);
              }
  ...
  const handleSmartArrange = () => {
      ...
      setGameState(prev => ({ ...prev, currentSuggestionIndex: nextIndex, currentArrangement: prev.aiSuggestions[nextIndex] }));
      setSelectedCardIds([]);
  };

  const submitHand = () => {
      const { top, middle, bottom } = gameState.currentArrangement;
      if (top.length !== 3 || middle.length !== 5 || bottom.length !== 5) { alert("请确保头墩3张、中墩5张、尾墩5张！"); return; }
      const pid = gameState.user ? `u_${gameState.user.id}` : TEMP_GUEST_ID;
      const currentTableId = gameState.tableQueue[gameState.currentTableIndex];
      submitPlayerHand(pid, {
          carriageId: gameState.currentCarriageId, tableId: currentTableId, seat: gameState.mySeat,
          hand: gameState.currentArrangement, timestamp: Date.now()
      });
      const nextIndex = gameState.currentTableIndex + 1;
      if (nextIndex >= 10) { alert("恭喜！本场次（车厢）所有对局已完成！请查看战报。"); handleShowSettlement(); } 
      else {
          ...
          setGameState(prev => ({
              ...prev, currentTableIndex: nextIndex, currentCards: nextCards,
              currentArrangement: suggestions[0], aiSuggestions: suggestions, currentSuggestionIndex: 0
          }));
          setSelectedCardIds([]);
      }
  };
  ...
  const MiniHandRow = ({ cards }: { cards: Card[] }) => (
      <div className="flex -space-x-4 items-center justify-center">
          {cards.map((c, i) => <CardComponent key={i} card={c} small className="w-8 h-12 text-[10px] shadow-sm ring-1 ring-black/10" />)}
      </div>
  );
      ...
      return (
          <div className="h-full w-full bg-green-950 flex flex-col overflow-hidden">
              <div className="shrink-0 p-4 bg-black/20 backdrop-blur-md border-b border-white/5 flex justify-between items-center z-10">
                   <div><h2 className="text-xl font-black text-yellow-400">战绩结算</h2><div className="text-xs text-white/50">共 {details.length} 局</div></div>
                   <div className="text-right"><div className="text-xs text-white/50">总盈亏</div><div className={`text-2xl font-mono font-bold ${totalScore >= 0 ? 'text-red-400' : 'text-green-400'}`}>{totalScore > 0 ? '+' : ''}{totalScore}</div></div>
              </div>
                                  ...
                                  return (
                                      <div key={pDetail.playerId} className={`relative p-2 rounded-xl border ${isMe ? 'bg-yellow-900/20 border-yellow-500/50' : 'bg-white/5 border-white/5'}`}>
                                          <div className="flex justify-between items-center mb-1">
                                              <span className={`text-xs font-bold truncate max-w-[80px] ${isMe ? 'text-yellow-400' : 'text-white/70'}`}>{pDetail.name}</span>
                                              <span className={`text-xs font-mono font-bold ${pDetail.score >= 0 ? 'text-red-400' : 'text-green-400'}`}>{pDetail.score > 0 ? '+' : ''}{pDetail.score}</span>
                                          </div>
                                          {pDetail.specialType ? (
                                              <div className="h-24 flex items-center justify-center bg-yellow-500/10 rounded-lg border border-yellow-500/30">
                                                  <div className="text-center"><div className="text-yellow-400 font-black text-lg drop-shadow-md">★ 特殊牌型 ★</div><div className="text-white font-bold text-sm mt-1">{pDetail.specialType}</div></div>
                                              </div>
                                          ) : (
              ...
              <div className="absolute bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-black to-transparent">
                  <button onClick={() => setGameState(INITIAL_STATE)} className="w-full py-4 bg-yellow-600 rounded-xl font-bold text-white shadow-lg active:scale-95 transition-transform">返回大厅</button>
              </div>
          </div>
      );
  };
  ...
  const renderSeatButton = (carriageId: number, seat: Seat, label: string, className: string) => {
      ...
      return (
          <button onClick={(e) => { e.stopPropagation(); handleSeatSelect(carriageId, seat); }}
              className={`absolute w-12 h-12 rounded-full border-2 flex flex-col items-center justify-center transition-all shadow-lg backdrop-blur-sm group ${className} ${isSelected ? 'bg-red-600 border-red-400 scale-110 shadow-red-500/50 z-20' : 'bg-black/40 border-white/10 hover:bg-yellow-600/80 hover:border-yellow-400 hover:scale-110'}`}>
  ...
  return (
    <div className="h-screen w-screen bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-green-800 to-green-950 font-sans flex flex-col overflow-hidden relative">
          ...
          <div className="absolute inset-0 z-[60] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
              <div className="bg-green-900 border border-yellow-500/30 w-full max-w-sm rounded-2xl p-6 shadow-2xl relative">
                  ...
                  <div className="space-y-4">
                      <div><label className="text-xs text-white/50 mb-1 block">手机号</label><input type="tel" className="w-full bg-black/40 border border-white/10 rounded-lg px-4 py-3 text-white outline-none focus:border-yellow-500" placeholder="请输入手机号" value={authForm.phone} onChange={e => setAuthForm({...authForm, phone: e.target.value})} /></div>
                      {showAuthModal === 'register' && <div><label className="text-xs text-white/50 mb-1 block">昵称</label><input type="text" className="w-full bg-black/40 border border-white/10 rounded-lg px-4 py-3 text-white outline-none focus:border-yellow-500" placeholder="游戏中显示的名称" value={authForm.nickname} onChange={e => setAuthForm({...authForm, nickname: e.target.value})} /></div>}
                      <div><label className="text-xs text-white/50 mb-1 block">密码 (至少6位)</label><input type="password" className="w-full bg-black/40 border border-white/10 rounded-lg px-4 py-3 text-white outline-none focus:border-yellow-500" placeholder="设置密码" value={authForm.password} onChange={e => setAuthForm({...authForm, password: e.target.value})} /></div>
      ...
      {showWalletModal && (
          <div className="absolute inset-0 z-[60] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
              <div className="bg-green-900 border border-yellow-500/30 w-full max-w-sm rounded-2xl p-6 shadow-2xl relative">
                   ...
                   <div className="space-y-4">
                       <div className="flex gap-2"><input type="tel" className="flex-1 bg-black/40 border border-white/10 rounded-lg px-4 py-2 text-white outline-none focus:border-yellow-500" placeholder="输入对方手机号" value={walletForm.searchPhone} onChange={e => setWalletForm({...walletForm, searchPhone: e.target.value})} /><button onClick={handleSearchUser} className="bg-white/10 px-4 rounded-lg hover:bg-white/20">搜索</button></div>
      ...
      {gameState.phase === GamePhase.SETTLEMENT_VIEW ? <ReportView /> : gameState.phase === GamePhase.LOBBY ? (
            ...
            <div className="w-full h-16 bg-black/20 backdrop-blur-md border-b border-white/5 flex items-center justify-between px-4 z-10 shrink-0">
                {gameState.user ? (
                    <button onClick={handleLogout} className="flex items-center gap-2 text-white/70 hover:text-white transition-colors">
                        <div className="w-8 h-8 rounded-full bg-green-600 flex items-center justify-center border border-white/10 font-bold text-xs">{gameState.user.nickname[0]}</div>
                        <div className="flex flex-col items-start"><span className="text-sm font-bold leading-none">{gameState.user.nickname}</span><span className="text-[10px] text-white/40">退出登录</span></div>
                    </button>
                ) : (
                    <button onClick={() => setShowAuthModal('login')} className="flex items-center gap-2 text-white/70 hover:text-white transition-colors">
                        <div className="w-8 h-8 rounded-full bg-white/10 flex items-center justify-center border border-white/10"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"></path></svg></div><span className="text-sm font-bold">登录/注册</span>
                    </button>
                )}
                <button onClick={handleShowSettlement} className="flex flex-col items-center group relative top-1"><span className="text-yellow-400 font-black text-lg leading-none tracking-wider group-hover:scale-110 transition-transform">我的战绩</span><span className="text-[10px] text-white/40 group-hover:text-white/60">已完成 {gameState.submissions.length} 局</span></button>
                <button onClick={() => { if(gameState.user) setShowWalletModal(true); else alert("请先登录"); }} className="flex items-center gap-2 bg-black/30 hover:bg-black/50 border border-yellow-500/30 rounded-full px-3 py-1.5 transition-all active:scale-95">
                   <div className="w-4 h-4 rounded-full bg-yellow-500 flex items-center justify-center text-[10px] text-black font-bold">$</div>
                   <div className="flex flex-col items-start leading-none"><span className="text-yellow-400 font-mono font-bold text-sm">积分管理</span>{gameState.user && <span className="text-[9px] text-white/60">{gameState.user.points}</span>}</div>
                </button>
            </div>
            ...
            <div className={`absolute bottom-0 left-0 right-0 p-6 bg-gradient-to-t from-black via-black/80 to-transparent transition-transform duration-300 ${lobbySelection ? 'translate-y-0' : 'translate-y-full'}`}>
                <div className="max-w-md mx-auto">
                    <button onClick={handleEnterGame} className="w-full bg-gradient-to-r from-red-600 to-red-500 hover:from-red-500 hover:to-red-400 text-white font-black text-xl py-4 rounded-2xl shadow-xl shadow-red-900/40 border border-red-400/30 active:scale-95 transition-all flex items-center justify-center gap-2">
                        <span>进入牌桌</span><span className="text-sm font-normal bg-black/20 px-2 py-0.5 rounded">{lobbySelection ? `${LOBBY_TABLES.find(t => t.carriageId === lobbySelection.carriageId)?.name} - ${lobbySelection.seat === 'North' ? '北' : lobbySelection.seat === 'South' ? '南' : lobbySelection.seat === 'West' ? '西' : '东'}` : ''}</span>
                    </button>
                    <p className="text-center text-white/40 text-xs mt-2">系统将自动检测场次人数，满足2人即可开赛</p>
                </div>
            </div>
            {!lobbySelection && <div className="absolute bottom-0 left-0 right-0 bg-black/30 backdrop-blur-md border-t border-white/5 py-4 px-4 text-center z-10"><p className="text-white/50 text-xs sm:text-sm font-medium tracking-wide">请选择结算时间场然后选择任意空位 · 至少2个玩家才能开始游戏</p></div>}
        </div>
      ) : (
        <div className="h-full w-full flex flex-col items-center p-2 max-w-3xl mx-auto overflow-hidden relative">
            <div className="w-full flex justify-between items-end px-4 pt-4 pb-2 border-b border-white/5 bg-green-950/50 backdrop-blur-sm z-20">
                <div><div className="flex items-center gap-2"><span className="bg-yellow-600 text-white text-[10px] font-bold px-1.5 rounded">{LOBBY_TABLES.find(t => t.carriageId === gameState.currentCarriageId)?.name || '未知场次'}</span></div><div className="text-[10px] text-white/30 mt-1">当前座位: <span className="text-yellow-400 font-bold">{gameState.mySeat === 'North' ? '北' : gameState.mySeat === 'South' ? '南' : gameState.mySeat === 'West' ? '西' : '东'}</span></div></div>
            ...
            <div className="absolute bottom-6 left-0 right-0 px-3 w-full flex justify-center z-50">
                <div className="bg-black/90 backdrop-blur-xl p-2.5 rounded-2xl border border-yellow-500/20 shadow-2xl flex flex-row items-center gap-3 w-full max-w-lg">
                    ...
                    <button onClick={submitHand} className="flex-1 bg-gradient-to-r from-yellow-600 to-yellow-500 hover:from-yellow-500 hover:to-yellow-400 text-green-950 font-bold text-sm sm:text-base py-3 rounded-lg shadow-lg transition-all active:scale-95 min-w-0">提交本局</button>
                </div>
            </div>
            <button onClick={() => setGameState(prev => ({ ...prev, phase: GamePhase.LOBBY }))} className="absolute top-20 right-4 text-[10px] text-white/20 p-2 hover:text-white">保存退出</button>
        </div>
      )}
