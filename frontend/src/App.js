...
export default function App() {
  ...
  if (!user) {
    if (view === 'login')
      return <Login onLogin={u => { setUser(u); setView('room'); }} onSwitch={() => setView('register')} />;
    else
      return <Register onRegister={u => { setUser(u); setView('room'); }} />;
  }
  if (roomId)
    return <GameTable roomId={roomId} user={user} onLeave={() => setRoomId(null)} />;
  return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-200 to-pink-100">
      <header className="p-4 flex justify-between items-center">
        ...
        <button className="btn" onClick={() => { setUser(null); setView('login'); }}>退出登录</button>
      </header>
      <main className="p-4">
        <Points user={user} />
